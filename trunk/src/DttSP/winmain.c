...
setup_defaults (unsigned int thread)
{
    ...
    sprintf (loc[thread].path.rcfile, "%s%0u_%0u", RCBASE, top[thread].pid,thread);
    sprintf (loc[thread].path.parm, "%s%0u_%0u", PARMPATH, top[thread].pid,thread);
    sprintf (loc[thread].path.meter, "%s%0u_%0u", METERPATH, top[thread].pid,thread);
    sprintf (loc[thread].path.spec, "%s%0u_%0u", SPECPATH, top[thread].pid,thread);
    sprintf (loc[thread].path.wisdom, "%s%0u_%0u", WISDOMPATH, top[thread].pid,thread);
