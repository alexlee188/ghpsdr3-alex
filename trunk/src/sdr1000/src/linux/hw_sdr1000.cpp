...
void SDR1000::SRLoad(unsigned char addr, unsigned char new_data) // parallel only
{
	...
	tmp_data = pio_data | SCLR_NOT; tmp_data |= (uint8) addr; tmp_data |= DCDR_NE; 
	Latch(PIO_IC01, tmp_data);		// Latch 2:4 decoder outputs
	tmp_data = pio_data | SCLR_NOT; tmp_data |= (uint8) addr;
	Latch(PIO_IC01, tmp_data);		// Take 2:4 decoder enable low
	tmp_data = pio_data | SCLR_NOT; tmp_data |= (uint8) addr; tmp_data |= DCDR_NE; 
	Latch(PIO_IC01, tmp_data);		// Take 2:4 decoder enable high
}

void SDR1000::WriteDDS(unsigned char reg_index, unsigned char new_data) // parallel only
{
  Latch(PIO_IC11, (uint8) new_data); // setup data bits
  Latch(PIO_IC08, (uint8) reg_index | DDSWRB); // setup address bits with WRB high
  Latch(PIO_IC08, (uint8) reg_index); // Send write command with WRB low
  Latch(PIO_IC08, (uint8) reg_index | DDSWRB); // take WRB back high
}
...
unsigned char SDR1000::StatusPort() // returns status port value
{
	if(usb)
...
void SDR1000::SetINAOn(bool b) // turns INA gain post mixer on
{
	if(b) pio_ic03->ClearBit(7);
	else pio_ic03->SetBit(7);
}

void SDR1000::SetATTOn(bool b) // turns on 10dB Attenuator
{
	if(rfe)
	{
                ATTNon = b;
...
void SDR1000::SetTRX_TR(bool b) // sets original TR relay
{
	if(b) pio_ic01->SetBit(6);
	else pio_ic01->ClearBit(6);
}

void SDR1000::SetRFE_TR(bool b) // sets RFE TR switches
{
	if(rfe)
	{
...
void SDR1000::SetPA_TR(bool b) // sets PA TR switches
{
	if(rfe & pa)
	{
...
void SDR1000::SetXVTR_TR(bool b) // sets XVTR TR switch
{
	if(rfe)
	{
...
void SDR1000::SetXVTR_RF(bool b) // sets XVTR RF relay
{
	if(rfe)
	{
...
void SDR1000::SetX2(unsigned char val) // valid int values [0, 0x7F] (7 bits)
{
  pio_ic03->SetData((pio_ic03->GetData() & 80) | (uint8) (val&0x7F));
}

void SDR1000::SetImpOn(bool b) // switches in the impulse response to the RF line
{
	if(rfe)
	{
...
void SDR1000::SetPA_Bias(bool b)
{
	if(rfe)
	{
...
void SDR1000::SetPA_ADC_CS_NOT(bool b)
{
	if(rfe)
	{
...
void SDR1000::SetPA_ADC_DI(bool b)
{
	if(rfe)
	{
...
void SDR1000::SetPA_ADC_CLK(bool b)
{
	if(rfe)
	{
...
void SDR1000::SetClockRefFreq(char freq) // acceptable values are selected integeral divisors of 200MHz
{
	switch(freq)
	{
...
unsigned char SDR1000::PA_ReadADC(unsigned char chan) // chan 0 = FWD, chan 1 = REV -- returns an 8 bit value
{
	if(rfe)
	{
		if(usb)
...
unsigned char SDR1000::ATU_Tune(unsigned char mode) // mode 0=bypass, 1=memory, 2=full -- returns 0 if successful
{
	rfe_ic11->ClearBit(7);
...
unsigned char SDR1000::ReadDDSReg(unsigned char reg_index)
{
  return (unsigned char) dds->Read((uint8) reg_index);
}

void SDR1000::WriteDDSReg(unsigned char reg_index, unsigned char new_data)
{
  dds->Write((uint8) reg_index, (uint8) new_data);
}
...
double SDR1000::FTW2Freq(unsigned long long ftw)
{
	return (double)(ftw*(200.0+freq_cal_offset)/0xFFFFFFFFFFFFllu);
}

unsigned long long SDR1000::Freq2FTW(double freq)
{
...
void SDR1000::SetSpurReductionMask(unsigned long long mask)
{
	double current_freq;
