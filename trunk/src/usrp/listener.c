...
void create_listener_thread() {
    ...
    if(rc<0) {
        perror("Listener: pthread_create listener_thread failed");
        exit(1);
    }
...
void* listener_thread(void* arg) {
    ...
    address.sin_port=htons(LISTEN_PORT); //LISTEN_PORT = 11000 as constant
    if(bind(s,(struct sockaddr*)&address,sizeof(address))<0) {
        perror("Listener: command socket bind failed");
        exit(1);
    }
